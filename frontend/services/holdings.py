...
class HoldingsMixin:
    ...
    # Dropped in the P0-5 split, which 500'd the perf chart; restored + extended
    # (P3-9) with an intraday 1D and a dynamic YTD. Alpaca uses "1A" for one year.
    _PERF_PERIODS = {
        "1D": ("1D", 2, "5Min"),   # intraday: portfolio only (SPY is daily)
        "1W": ("1W", 10, "1D"),
        "1M": ("1M", 35, "1D"),
        "3M": ("3M", 100, "1D"),
        "1Y": ("1A", 370, "1D"),
        "YTD": (None, None, "1D"),  # resolved dynamically from Jan 1
    }
    ...
    def get_portfolio_performance(self, period: str = "1M") -> Dict[str, Any]:
        ...
        alpaca_period, spy_days, timeframe = self._resolve_period(period)
        ...
        try:
            ...
            for ts, eq in zip(hist.timestamp or [], hist.equity or []):
                ...
                if intraday:
                    ...
                    # one point per day (last wins) keeps daily ranges clean
        ...
        # --- SPY closes (daily) — skipped for intraday, where a single daily
        #     close would render as a misleading flat benchmark. ------------
        spy_by_date = {}
        if not intraday:
            try:
